...
`squat-counting` 是复用现有人体姿态模型的业务场景，不需要额外模型制品。可选择已就绪的 RTMO、RTMPose、YOLO-Master Pose、YOLO11 Pose 或 YOLO26 Pose。

- 上传视频：`POST /api/ai/squat/video`，完成后通过进度接口取得统计摘要和带标注 MP4。
- 本地摄像头：创建 `local` 实时会话后逐帧提交 JPEG；服务端返回标注帧和计数状态。
- 网络摄像头：只能选择 `Camera` 表中已登记且启用的视频源；服务端拉流并提供 MJPEG 标注流。
- 默认规则：站立角度 `≥160°`、下蹲角度 `≤100°`、连续 3 个有效采样确认。
- 只统计面积和腿部关键点质量最优且能延续跟踪的单一主训练者。
- 本地和网络实时会话不录制视频，停止或超时后只保留会话摘要。
- 机位应能持续看见髋、膝、踝；半蹲、初始蹲姿和未完整起身不会计数。
