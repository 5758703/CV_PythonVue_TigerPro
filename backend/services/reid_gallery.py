...
def invalidate_faiss(model_key: str | None = None, modality: str | None = None):
    with _lock:
        if model_key is None and modality is None:
            _faiss_cache.clear()
            return
...
def _build_faiss_index(model_key: str, modality: str = "appearance"):
    faiss = _import_faiss()
    person_ids, names, mat, face_person_ids = get_gallery(model_key, modality)
    if mat.size == 0 or not person_ids:
        dim = 512
...
def match_embedding_faiss(
    ...
    try:
        with _lock:
            ...
            if cached is None:
                cached = _build_faiss_index(model_key, modality)
