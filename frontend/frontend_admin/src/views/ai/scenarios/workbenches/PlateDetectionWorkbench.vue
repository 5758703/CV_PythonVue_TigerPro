  ...
  <div class="wb-grid">
    <section class="wb-controls" aria-label="车牌检测输入与参数">
      ...
      <aside class="wb-capability-note">
        <strong>Detector only</strong>
        <p>该场景只定位车牌，不把检测框冒充字符识别结果。</p>
        <RouterLink to="/ai/paddleocr">将裁剪图送入 OCR <span aria-hidden="true">→</span></RouterLink>
      </aside>
    ...
    <section class="wb-canvas" aria-label="车牌检测画布">
    ...
    <ResultPanel
      ...
      :filename="`${scenario.modelKey}-plates`"
    >
      <div v-if="normalizedResult.detections.length" class="wb-result-content">
        <div class="wb-metrics">
          ...
          <div><span>OCR</span><strong>{{ hasOcr ? '已返回' : '未联动' }}</strong></div>
        </div>
          ...
          <table class="wb-table">
            ...
            <tbody>
              <tr v-for="(item, index) in normalizedResult.detections" :key="index" :class="{ 'is-selected': selectedIndex === index }" tabindex="0" @click="selectDetection(index)" @keydown.enter="selectDetection(index)" @keydown.space.prevent="selectDetection(index)">
                ...
                <td>{{ ocrText(item) || '未返回' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <p v-if="!hasOcr" class="wb-result-note">响应中没有 OCR 字段；当前结果仅证明车牌已定位。</p>
      </div>
...
const validationErrors = computed(() => validateWorkbenchState('plate_detection', { file: file.value, conf: conf.value, imgsz: imgsz.value }, props.scenario.input))
...
const hasOcr = computed(() => normalizedResult.value.detections.some((item) => Boolean(ocrText(item))))

function ocrText(item) {
  return [item?.ocrText, item?.plateText, item?.text].find((value) => typeof value === 'string' && value.trim()) || ''
}
...
async function runInference() {
  ...
  resetResult()
  try {
    const form = serializeScenarioForm('plate_detection', { file: file.value, conf: conf.value, imgsz: imgsz.value })
    ...
    normalizedResult.value = normalizeWorkbenchResult('plate_detection', response.data?.result)
    await nextTick()
    drawOverlay()
    createCrops()
    emit('completed', response.data)
  } catch (requestError) {
    error.value = requestError?.response?.data?.message || requestError?.message || '车牌检测失败，请检查输入与运行环境。'
  } finally {
    busy.value = false
  }
}
